$(document).ready(function(){
        ...
        $("#weather li").each(function(index){
            $.ajax({
                ...
                success : function(data){
                    ...
                    console.log(this); // {url: 'https://api.openweathermap.org/data/2.5/weather?q=Vancouver&appid=1451c23e29ef1202473d5f41f30131c2', type: 'GET', isLocal: false, global: true, processData: true, …}
                    // $("#weather ul").append(w_box);
    ...
    */
    ...
    $(document).on("click", ".cities button", function(){
        ...
        // city.push($city_txt); // 클릭한 버튼의 텍스트를 city 라는 배열 데이터의 마지막 자리에 위치를 시킨다. ==> 마지막에 클릭한 버튼의 데이터 정보는 리스트 중에서 맨 마지막에 나온다.
        city.unshift($city_txt); // 클릭한 버튼의 텍스트를 city 라는 배열 데이터의 첫번째 자리에 위치를 시킨다. ==> 마지막에 클릭한 버튼의 데이터 정보는 리스트 중에서 맨 처음에 나온다.
        console.log(city);
